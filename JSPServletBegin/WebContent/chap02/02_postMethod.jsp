...
<body>
	<%-- <form action="/JSPServletBegin/02_postMethod.do"> 또는 --%>
	...
	<%-- form 태그에서 submit(전송)을 하면 
		 http://localhost:9090/JSPServletBegin/02_postMethod.do 으로 데이터를 전송시킨다.
	 	 ...
	 	 POST 방식은 웹브라우저 주소창에 http://URL주소만나옴 으로 나타내주는 것이며 ? 가 없다.
		 POST 방식은 전송할 데이터를 웹브라우저 주소창이 아닌 본문에 넣어서 보낸다.
		 POST 방식은 웹브라우저 주소창에 데이터를 공개하지 않으므로 보안성은 좋지만 속도는 GET 방식보다 느리다.
		 그래서 일반적으로 보안과 관련없는 데이터 입력, 수정(예: 회원가입, 내정보수정) 와 같은 insert, update 절에서 주로 사용된다.		 
	 --%>
		<fieldset>
