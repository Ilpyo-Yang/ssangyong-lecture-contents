...
public class Gujikja {
	...
	String passwd;
	String name;
	String jubun;   //주민번호 7자리		예:9501151 남자	9501152 여자 
					//주민번호 7자리		예:0101153 남자	0101154 여자
	
	static int count; //gujikja 객체의 개수를 알아오려는 용도
	
	// method 생성 (=> field(속성)을 어떤 방식으로 처리하고자 할 때 메서드를 생성)
	// == 구직자의 성별을 알아오는 메서드 생성 ==
	String getGender() {
		String gender = "";		
	...
	int getAge(){
		...
		if("1".equals(sgenderNum) || "2".equals(sgenderNum)) {
			age = year-(Integer.parseInt("19"+jubun.substring(0,2)))+1;
		} else {
			age = year-(Integer.parseInt("20"+jubun.substring(0,2)))+1;
		}
		return age;
	}
