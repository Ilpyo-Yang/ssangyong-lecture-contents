...
/*
 	...
 	그리고 배열 또는 객체라는 사실을 꼭 기억하도록 하자!!
 */
		
		//1. == 배열의 선언 ===
		//int subjectArr[]; 와 동일하나 주로 사용되는 것은 아래 방식
			int[] subjectArr;
		...
		/*
		 	...
		 	|0|1|2|3|4|5|6|
		 	---------------
		 	위의 숫자는 배열의 인덱스를 가리키는 번호로써 0부터 시작하여 1씩 증가한다. 
		 	...
		 	정수는 0, 실수는 0.0, char는 ' ', String을 포함한 객체는 null 이 들어온다. 
		 */
		...
		double avg = Math.round((double)sum/subjectArr.length*10)/10.0;   //***계산식에 유의!!!
...
/*
 	위의 1. == 배열의 선언 === 과 2. == 선언되어진 배열을 메모리에 할당을 해준다. == 을
 ...
 */
			...
			avg = Math.round((double)sum/subjectArr2.length*10)/10.0;   //***계산식에 유의!!!
			System.out.println("평균 : " +avg);
			//평균 : 87.6
...
/*
 	위의 1. == 배열의 선언 === 과 2. == 선언되어진 배열을 메모리에 할당을 해준다. == 과
 	3. == 데이터값을 넣어주기 == 을 아래와 같이 동시에 할 수 있다.		 	
 */
				...
				avg = Math.round((double)sum/subjectArr3.length*10)/10.0;   //***계산식에 유의!!!
				System.out.println("평균 : " +avg);
				//평균 : 87.6
 ...
 */
				...
				avg = Math.round((double)sum/subjectArr4.length*10)/10.0;   //***계산식에 유의!!!
				System.out.println("평균 : " +avg);
				//평균 : 87.6
				...
				for(int subject : subjectArr4) {
					...
					// 반복하면서 배열방에 들어있던 값을 차례차례 int subject 변수 속에 넣어준다.
