	...
	//	System.out.println("co1.isUseCompany() => " + co1.isUseCompany());
		...
		do {
			...
			if (gu==null && co==null)
				...
				title = "======  >>  메인메뉴 [구직자 "+gu.getName()+"로그인 중..] <<  ========\n";
			else if (gu==null && co!=null)
				title = "======  >>  메인메뉴 [구인회사 "+co.getName()+"로그인 중..] <<  ========\n";
			...
			switch (smenuNo) {
				...
				case "3": // 구직자 로그인
					if (co==null && gu!=null)
						System.out.println("\n현재 구직자 "+gu.getName()+"로 로그인되어 있습니다. 로그아웃 후 실행해 주세요!");
					else if (co!=null && gu!=null)
						System.out.println("\n현재 구인회사 "+co.getName()+"로 로그인되어 있습니다. 로그아웃 후 실행해 주세요!");
					else 
				...
				case "8": // 내정보 변경하기
					if(gu!=null && co==null)
						gu = ctrl.updateMyInfo(sc, gu); // 구직자로 로그인되어진 상태
					else if(co!=null && gu==null)
						co = ctrl.updateMyInfo(sc, co); // 구인회사로 로그인되어진 상태
					else
						System.out.println("\n>> 먼저 로그인 하셔야 합니다. <<\n");
					break;	
