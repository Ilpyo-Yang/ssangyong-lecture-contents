...
//	public boolean duplicateIdGujikja(String id, Member[] mbrArr) {
	public boolean  duplicateIdGujikja(String id, List<Member> mbrList) {	
	...
	*/	
		for(int i=0; i<mbrList.size(); i++) {
			if(mbrList.get(i) instanceof Gujikja) {
				String storedId = mbrList.get(i).getId();
				if(storedId.equals(id)) {
					bool = true;
					break;
				}
...
//	public boolean duplicateIdCompany(String id, Member[] mbrArr) { 
	public boolean duplicateIdCompany(String id, List<Member> mbrList) {	
...
//	public boolean register(Scanner sc, Member[] mbrArr, int n) { 
	public List<Member> register(Scanner sc, List<Member> mbrList, int n) {	
	...
	*/	
		...
		result = mbrList.add(mbr);
		if(result && n==1) {
			System.out.println(">> 구직자로 회원가입 성공!! <<");
		} else if (result && n==2) {
			System.out.println(">> 구인회사로 회원가입 성공!! <<");
		} else
...
//	public Member login(Scanner sc, Member[] mbrArr, int n) {
	public Member login(Scanner sc, List<Member> mbrList, int n) {	
	...
	// 모든 회원정보 출력(구직자 , 구인회사)
	@Override
	public void viewInfoAll(List<Member> mbrList, int n) {
	// int n 은 n값이 1이면 구직자, n값이 2이면 구인회사.
		...
		*/	
			for(int i=0; i<mbrList.size(); i++) {
				if(mbrList.get(i) instanceof Gujikja) {
					System.out.printf("%-12s\t%-15s\t%-6s\t%-4d\t%-2s\n",mbrList.get(i).getId(), mbrList.get(i).getPasswd(), mbrList.get(i).getName(), ((Gujikja) mbrList.get(i)).getAge(), ((Gujikja) mbrList.get(i)).getGender()); 
				}
			}
		...
		*/	
			for(int i=0; i<mbrList.size(); i++) {
				if(mbrList.get(i) instanceof Company) {
					System.out.printf("%-12s\t%-15s\t%-6s\t%-6s\t%-10d\n",mbrList.get(i).getId(), mbrList.get(i).getPasswd(), mbrList.get(i).getName(), ((Company) mbrList.get(i)).getJobType(), ((Company) mbrList.get(i)).getSeedMoney()); 
				}
			}
		} // end of else if(n==2)----------------------
