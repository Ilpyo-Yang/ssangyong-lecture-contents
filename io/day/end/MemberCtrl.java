...
public class MemberCtrl implements InterMemberCtrl {
	...
	// 구인회사 중복아이디 검사 
	@Override
	public boolean duplicateIdCompany(String id, ArrayList<Member> mbrList) {

		boolean bool = false;
		
		for(int i=0; i<Member.count; i++) {
			if(mbrList instanceof Company) {
				String storedId = mbrList[i].getId();
				if(storedId.equals(id)) {
					bool = true;
					break;
				}
	...
	// 회원가입(구직자 , 구인회사)
	@Override
	public boolean register(Scanner sc, ArrayList<Member> mbrList, int n) {
	// int n 은 n값이 1이면 구직자로 회원가입, n값이 2이면 구인회사로 회원가입.	
		...
		if(n==1) {
		 // 구직자로 회원가입
			...
			do {
				...
				if(id != null) {
					boolean bool = duplicateIdGujikja(id, mbrList);
		 ...
		 // 구인회사로 회원가입	
			...
			do {
				...
				if(id != null) {
					boolean bool = duplicateIdCompany(id, mbrList);
		...
		if(Member.count < mbrList.size()) {
			mbrList[Member.count++] = mbr;
			result = true;
		}
	...
	@Override
	public Member login(Scanner sc, ArrayList<Member> mbrList, int n) {
		...
		if(n==1) {
			...
			for(int i=0; i<Member.count; i++) {
				...
				if(mbrList[i] instanceof Gujikja) {
					if(id.equals(mbrList[i].getId()) && 
					   passwd.equals(mbrList[i].getPasswd())	) {
			...
			for(int i=0; i<Member.count; i++) {
				...
				if(mbrList[i] instanceof Company) {
					if(id.equals(mbrList[i].getId()) && 
					   passwd.equals(mbrList[i].getPasswd())	) {
	...
	// 모든 회원정보 출력(구직자 , 구인회사)
	@Override
	public void viewInfoAll(ArrayList<Member> mbrList, int n) {
	...
	// 회원정보 변경(구직자 , 구인회사)
	@Override
	public Member updateMemberInfo(Scanner sc, ArrayList<Member> mbrList) {
	...
	// 파일로 저장하기
	@Override
	public void saveFile(ArrayList<Member> mbrList) {
		String fileName = "c:/iodatatest";
		byte[] byteArr = new byte[10];
		int input = 0;
		
		try {
			FileOutputStream fo = new FileOutputStream(fileName, true);
			for ((input = fo.read(mbrList)) != -1) {
	...
	// 회원정보 변경(구직자 , 구인회사)
	@Override
	public void openFile(ArrayList<Member> mbrList) {
