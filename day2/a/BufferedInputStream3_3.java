...
public class BufferedInputStream3_3 {
...
*/
	...
	public static void main(String[] args) {
		...
		try {
			...
			String srcFileName = "c:/iotestdata/원본/JDK-8u271설치파일및매뉴얼.zip";
			File srcFile = new File(srcFileName);
			FileInputStream fist = new FileInputStream(srcFileName);
			
			BufferedInputStream bist = new BufferedInputStream(fist, 1024*1024);		//1024 == 1kb
			// 노드스트림인 fist에 필터스트림(보조스트림)을 장착함.
			// fist [필터스트림(보조스트림)]의 버퍼크기는 1024*1024 byte로 했다.
			
			// 출력노드스트림 ==> 파일(FileOutputStream.out)
			// 출력노드스트림 ==> 보조(필터) 스트림(BufferedOutputStream)을 장착한다.
			
			String targetFileName = "c:/iotestdata/복사본/JDK-8u271설치파일및매뉴얼_복사본.zip";
			...
			// bost2 [필터스트림(보조스트림)]의 버퍼크기는 1024*1024 byte로 했다.
