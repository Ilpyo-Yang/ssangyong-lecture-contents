...
public class FrontControllerServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		/*
		        ...
		        그러므로 init(ServletConfig config) 메소드에는 FrontControllerFrontControllerServlet 서블릿이 동작해야할 환경설정을 잡아주는데 사용된다.
	    */
			// *** 확인용 *** //
	...
	//	System.out.println("~~~ 확인용 props => "+props);
		...
		try {
			...
			// 파일의 경로가 잘못 될 경우가 있기 때문에 예외처리를 해준다.
	        ...
	        */
			...
			while(en.hasMoreElements()) {// key값이 있다면
			...
			//	System.out.println("### 확인용 value =>" + pr.getProperty(key));
				...
				*/
				...
				if(className != null) {
					...
					// Command properties 에 공백을 입력하게 되면 그 공백을 없애주는 과정이다.
