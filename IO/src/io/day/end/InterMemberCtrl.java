...
public interface InterMemberCtrl {
	...
	// 구인회사 중복아이디 검사 
	boolean duplicateIdCompany(String id, ArrayList<Member> mbrList);
	// 아이디가 이미 존재한다라면 true 를 리턴.
	// 아이디가 존재하지 않는다라면 false 를 리턴.
	
	
	// 회원가입(구직자 , 구인회사)
	boolean register(Scanner sc, ArrayList<Member> mbrList, int n);
	// int n 은 n값이 1이면 구직자로 회원가입, n값이 2이면 구인회사로 회원가입.
	
	
	// 로그인(구직자 , 구인회사)  
	Member login(Scanner sc, ArrayList<Member> mbrList, int n); 
	// int n 은 n값이 1이면 구직자로 로그인, n값이 2이면 구인회사로 로그인.
	
	
	// 모든 회원정보 출력(구직자 , 구인회사)
	void viewInfoAll(ArrayList<Member> mbrList, int n);
	...
	// 파일에서 불러오기
	void saveFile(ArrayList<Member> mbrList);
	
	// 회원정보 변경(구직자 , 구인회사)
	void openFile(ArrayList<Member> mbrList);
}
