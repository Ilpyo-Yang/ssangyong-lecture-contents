...
public class EnumMain_2 {
	...
	public static void main(String[] args) {
		...
		// values() 메서드는 enum(열거형)의 모든 상수를 배열에 담아 반환해주는 것이다.
		
		System.out.println("1. ~~~~~~~~~~~~~~~~~~~~~~\n");
		
		for (SeasonType_1 season :  seasonArr ) {
			String seasonName = season.name();
			// name() 메서드는 enum(열거형) 상수의 값을 문자열(String)로 반환해주는 것이다.
		...
		try {
		SeasonType_1 season1 = SeasonType_1.valueOf("여름");
		// valueOf()메소드는 열거형 SeasonType_1 에서 문자열 "여름" 과 일치하는 열거형 상수를 반환해주는 것이다.
		
		System.out.println(season1);
		// 여름
		
		season1 = SeasonType_1.valueOf("한여름");
		System.out.println(season1);
